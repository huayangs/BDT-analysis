...
class MyCreate_A_HZ_tatall: public MyDebug{
public:
    //Constructor and Destructor
    MyCreate_A_HZ_tatall(int mode, string file_label, vector<string> file_name, vector<double> crosssection, vector<int> _nfiles , vector<string> directory): _mode(mode), _file_label(file_label), _file_name(file_name), _directory(directory), _crosssection(crosssection), _nfiles(_nfiles), _this_cathegorie("A_HZ_tatall") {};
    ~MyCreate_A_HZ_tatall() {;}
...
void MyCreate_A_HZ_tatall::Create()
{
    ...
    double mll, mtata, mlltata, ptl1, ptl2, ptta1, ptta2, etmiss, eventht, weight, type;
    double category = 1;
    ...
    //if (_mode==1)
    //{
    //    path_to_file = "/home/fkling/work/MG5_aMC_v2_3_2_2/Proc-Exotic/Signal_HZ_tatall/Results/";
    ...
    tree.Branch("var_mtata",&mtata, "var_mtata/d");
    tree.Branch("var_mlltata",&mlltata, "var_mlltata/d");
    tree.Branch("var_ptl1",&ptl1, "var_ptl1/d");
    tree.Branch("var_ptl2",&ptl2, "var_ptl2/d");
    tree.Branch("var_ptta1",&ptta1, "var_ptta1/d");
    tree.Branch("var_ptta2",&ptta2, "var_ptta2/d");
    ...
    for (int j=0; j<filename_in.size();j++)
    {
        ...
        type    = type_in[j];
        //only half the events will be used for tsting, the rest is for training and is not used for calulating significance
        ...
        for (Int_t entry=0; (entry<numberOfEntries) ; entry++)
        {
            ...
            if (debug()>13) cout << "                   number of taus:           "<< del_tau.size()<<endl;
            if (debug()>13) cout << "                   number of anti-taus:      "<< del_atau.size()<<endl;
            ...
            fastjet::PseudoJet del_tau1;
            fastjet::PseudoJet del_tau2;
            bool pass=false;
            
            //b,tau veto
            if (del_b.size()>0) continue;
            if (del_tau.size()!=1) continue;
            if (del_atau.size()!=1) continue;
            ...
            //read vars
            mll  = (del_l1+del_l2).m();
            mtata  = (del_tau1+del_tau2).m();
            mlltata= (del_l1+del_l2+del_tau1+del_tau2).m();
            ptl1 = del_l1.pt();
            ptl2 = del_l2.pt();
            ptta1 = del_tau1.pt();
            ptta2 = del_tau2.pt();
